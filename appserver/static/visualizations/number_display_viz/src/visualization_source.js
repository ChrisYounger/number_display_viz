...
function(
    ...
    // add trellis 
    // add new type that fills all available space
    var vizObj = {
        initialize: function() {
...
// Then set borders around the place to be a few shades darker than the elem 
 ...
 // Try to make all these thesame size, fit in 100 by 100
                        ...
                        viz.svgString = '<rect x="2.5" y="2.5" width="90" height="45" class="number_display_viz-shape" />';
...
"<defs><pattern id='a' patternUnits='userSpaceOnUse' width='100' height='100'><rect x='0' y='0' width='100' height='100' fill='#F29E03'  class='number_display_viz-fill_primary'/><rect x='0' y='0' width='100' height='50' fill-opacity='0.2' fill='#ffa61d'  class='number_display_viz-fill_secondary'/></pattern></defs><rect x='0' y='0' width='1000' height='1000' fill='url(#a)'/></svg></pattern></defs>";
                    ...
                    if (viz.config.shapeshadow === "yes") {
                        viz.$svg.css("filter", "drop-shadow(" + tinycolor(viz.config.shapedropcolor).setAlpha(.5).toRgbString() + " 0px 0px " + (viz.size * viz.config.mainHeight * 0.03) + "px)");
                    }
                }
            }
...
var grd = viz.ctx2.createLinearGradient(0, (viz.size * viz.config.mainHeight) / 2, (viz.size * viz.config.mainWidth), (viz.size * viz.config.mainHeight) / 2);
grd.addColorStop(0.000, viz.getColorFromMode(viz.config.colorprimarymode, viz.config.colorprimary, value_color));
grd.addColorStop(1.000, viz.getColorFromMode(viz.config.colorsecondarymode, viz.config.colorsecondary, value_color));

                    viz.donutCfg.data.datasets[1].backgroundColor = [grd, viz.config.shadowcolor]; // value_color
